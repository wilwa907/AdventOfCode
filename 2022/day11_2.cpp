...
int lineNumber{0}, monkey;
long long newItem, monkeyBusiness{0}, maximumValue2, maximumValue1;
vector<int> monkeys;
vector<long long> worry;
vector<vector<long long>> worryLevel; // 2D-vector
const int lcm = 3*13*2*11*5*17*19*7; // Int used to keep worry levels manageable

long long MovedItems(long long item, int toMonkey, int fromMonkey) // Function that handles the updates of moved items
{
...
void day11()
{
    ...
    {
        newItem=0;
        worryLevel[0][i] = worryLevel[0][i] % lcm;
    ...
    {
        newItem=0;
        worryLevel[1][i] = worryLevel[1][i] % lcm;
    ...
    {
        newItem=0;
        worryLevel[2][i] = worryLevel[2][i] % lcm;
    ...
    {
        newItem=0;
        worryLevel[3][i] = worryLevel[3][i] % lcm;
    ...
    {
        newItem=0;
        worryLevel[4][i] = worryLevel[4][i] % lcm;
    ...
    worryLevel[4].clear(); // Clear monkey 0:s items since all is thrown
    
    // Throwing monkey 5:s items
    for (int i=0; i<worryLevel[5].size(); i++)
    {
        newItem=0;
        worryLevel[5][i] = worryLevel[5][i] % lcm;
    ...
    {
        newItem=0;
        worryLevel[6][i] = worryLevel[6][i] % lcm;
    ...
    {
        newItem=0;
        worryLevel[7][i] = worryLevel[7][i] % lcm;
    ...
    for (int k=1; k<10000; k++) // 10000 rounds (1 + 9999)
    {
        // Monkey 0
        for (int i=0; i<worryLevel[0].size(); i++){
            monkey0++;
